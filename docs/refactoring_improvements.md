...
### 2.7 완료된 코드를 다시 리뷰해서 핵심 목표가 실제로는 깨져 있던 걸 발견 (Phase 3b 안정화 패치)

Phase 3b를 닫은 뒤, 다음 Phase로 넘어가기 전에 지금까지 쌓인 코드를 다시 리뷰받았다. 가장 값어치 있었던 발견은 이 리팩토링의 핵심 목표("서버 없이 동작") 자체가 조용히 깨져 있었다는 것이었다.

**발견한 문제**: `app_config.json`이 없거나 `apiBaseUrl`이 비어 있으면 `loadAppConfig()`가 그 자리에서 실패를 반환했고, `initializeAuthServices()`는 그 실패를 보자마자 함수 전체를 리턴해버렸다. 그런데 `OnvifLiteClient`/`DeviceService`/`CctvControlService` — 서버와 전혀 무관한, 카메라 전용 서비스들 — 이 전부 그 함수 안, 그 리턴 지점 뒤에서 생성되고 있었다. 결과적으로 서버 설정이 없으면 카메라 서비스 자체가 만들어지지 않아, 게스트 버튼을 눌러도 "DeviceService 미연결" 상태가 됐다. 개발 중에는 계속 잘 동작하는 것처럼 보였는데, 알고 보니 이 개발 머신에 예전 팀 서버 주소가 담긴 `app_config.json`이 그냥 남아있어서 이 문제 자체가 가려져 있었던 것뿐이었다.

**고친 방법**: 서버 의존 서비스(`AuthService`/`PlaybackService`/`UgvService`/`EventService`/`WsClient`)와 카메라 도메인 서비스를 같은 함수 안에서 명확히 순서를 나눠, 후자는 서버 설정 성공 여부와 무관하게 항상 생성되도록 재배치했다. 검증을 위해 실제로 `app_config.json`을 다른 폴더로 치워보고, 게스트 → 디바이스 조회 → RTSP 재생 → PTZ 제어까지 전부 서버 설정 없이 되는지 직접 테스트했다.

같은 리뷰에서 WS-Security 타임스탬프 형식 오류(`Z`가 중복으로 붙는 문제), SOAP 요청의 XML escape 누락, 화면 재진입 시 자격증명 모달 취소가 오히려 Main으로 진입해버리는 버그, 로그인 화면이 설정 오류 상태를 로그아웃/게스트 복귀 후에 잃어버리는 버그 등도 같이 발견해서 고쳤다. 반면 SOAP 파싱을 정규식에서 `QXmlStreamReader`로 바꾸는 일이나 자동 테스트 도입처럼 스코프가 큰 항목은, 지금 단계(mock 대상 검증)에서는 우선순위가 아니라고 판단해 실제 카메라 연동 시점으로 명시적으로 미뤄뒀다.

### 2.8 이벤트도 서버 없이 직접 수신 — 기존 파서를 재사용해서 "지저분한 실데이터 대응력"은 그대로 유지 (Phase 4)

팀 프로젝트 때 CCTV가 별도 수신자 지정 없이 UDP로 이벤트를 브로드캐스트하고 있었던 걸 기억하고 있었는데, 착수 전에 이게 정확한 기억인지, 그리고 그때 이벤트 파싱이 지저분했던 것도 재현해야 하는지가 먼저 확인해야 할 질문이었다.

**기억이 맞다는 증거가 이미 코드에 있었다**: `EventService::buildEventInfo()`가 eventId/timestamp/channel/type 하나하나를 여러 후보 필드명 중 있는 걸로 채우고, 메시지의 wrapping 구조도 `payload.data`/`payload.events`/`event`(단일) 등 거의 모든 형태를 다 시도하도록 짜여 있었다 — 이 방어적인 코드 자체가 그때 실제로 겪었던 지저분함의 증거였다.

**결정**: 목업(Phase 0에서 이미 만든 `event_broadcaster.cpp`)까지 일부러 지저분하게 만들 필요는 없다고 판단했다 — 그 지저분함은 "카메라 → 서버" 구간의 것이었고, 지금은 서버를 안 거치고 카메라 브로드캐스트를 VMS가 직접 받는 구조라 재현 대상이 애매했다. 대신 목업은 깨끗한 JSON 그대로 두고, `EventService`에 이미 있는 방어적 파싱 로직을 재사용하는 신규 `LocalEventListener`를 만들었다 — "지저분한 실데이터에도 안전"이라는 능력은 잃지 않으면서 목업을 흉하게 만드는 수고는 안 하는 쪽으로 갔다.

**부가 발견**: 조사하다 보니 `EventService`도 서버 설정이 없으면 아예 생성되지 않는, 2.7절과 같은 종류의 문제가 있었다. 카메라 도메인 서비스들과 같은 구간으로 옮겨서 서버 없이도 이벤트 UI가 항상 동작하게 고쳤다.

**게이트 테스트 중 자기 실수를 발견한 순간도 있었다**: 사용자가 "예전에는 이벤트가 몇 초 간격으로 배치되어 업데이트됐던 것 같다"고 물었을 때, 새로 만든 서비스 계층(`LocalEventListener`/`EventService`)만 확인하고 "배치 로직 없다"고 잘못 답했다. 다시 짚어보니 화면 계층(`MainScreen`, 설계 원칙상 손대지 않은 기존 코드)에 정확히 그 디바운스 로직(5초 싱글샷 타이머 + burst 5개 이상이면 즉시 반영하는 안전장치)이 이미 있었다. 화면 계층을 안 건드렸기 때문에 그대로 살아있었던 것 — 리뷰 문서가 요구했던 "burst 상황에서 매번 재렌더링하지 않는다"는 이미 만족되어 있어서 새로 만들 게 없었다.
...
| 이벤트 수신 | 카메라 → 서버 → WS relay 필수 | 카메라(목업) → VMS가 UDP로 직접 수신, 서버 있으면 WS와 병행(같은 캐시 공유로 중복 자동 제거) |
