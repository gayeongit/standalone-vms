...
| 3b | 로컬 자격증명 캐시 | Phase 1 | 완료 (2026-09-16, 안정화 패치 2026-09-17) |
| 4 | 이벤트 직접 수신 경로 | Phase 0 | 완료 (2026-09-17) |
| 5 | Playback (mediamtx 기반, 더미 콘텐츠) | Phase 1 | **대기 (다음 작업, 선택)** |
...
**틀린 자격증명 UX (2026-09-16 추가 결정).** 처음엔 틀린 ID/PW로도 Main에 진입시키고 그 채널만 `연결 안 됨` 상태로 남기는 방식이었는데, 실제로 써보니 "선택된 채널처럼 보이는데 계속 깨져 있는" 모양이 UX상 어색했다. 재시도 모달 같은 새 UI를 추가하는 대신, 모달에서 새로 입력한 자격증명만 대표 채널 PTZ로 `RelativeMove(delta=0)` ping을 날려 즉시 검증하고, 틀리면 기존 "RTSP 조회 실패" 채널과 동일하게 그리드 배정 자체에서 제외하는 방식으로 변경 — 화면 계층(`main_screen.cpp`)은 안 건드리고 `finalize()` 이전 단계에서 걸러낸다.

**상태.** 완료 (2026-09-16). 게스트 채널 선택+재생/PTZ, 틀린 자격증명 거부(+UX 보정) 확인 완료. 로그인 상태의 QtKeychain 영구 저장 테스트는 실제/목업 인증 서버가 없어 이 환경에서 검증 불가 — 이 프로젝트의 목적이 "서버 없이 동작"인 만큼 우선순위가 아니라 판단해 의도적으로 스킵. 세부 내용은 `docs/dev_execution_plan.md`의 Phase 3b 섹션 참고.

**안정화 패치 (2026-09-17).** Phase 3b 완료 후 코드 리뷰(`docs/code_review_stabilization_plan.md`)를 받아 Phase 4 진입 전 안정화 작업을 진행 — 서버 설정(`app_config.json`) 없이도 로컬 카메라(ONVIF) 서비스가 항상 초기화되도록 수정(이전엔 `apiBaseUrl`이 없으면 `DeviceService` 자체가 안 만들어져서 "서버 없이 동작"이라는 핵심 목표가 깨져 있었음), WS-Security `Created` 중복 `Z`/XML escape 누락 수정, 순환 참조·코덱 캐시 누락 등 잡버그 정리, "장치 관리" 탭(런타임과 분리된 no-op) 비활성화. 게이트 테스트 중 추가로 자격증명 모달 취소 버그, 로그인 화면 상태 유지 버그 등 4건을 더 발견해 같이 수정. 세부 내용은 `docs/dev_execution_plan.md`의 "Phase 3b 안정화 패치" 섹션 참고. **완료 (2026-09-17).**
...
**진입 전 조사 (2026-09-17).** 착수 전에 "팀 프로젝트 때 CCTV가 진짜 브로드캐스트로 이벤트를 날렸는지, 그때 파싱이 지저분했던 기억이 맞는지" 확인 — `EventService::buildEventInfo()`(`src/services/event_service.cpp`)가 이미 eventId/timestamp/channel/type 전부 여러 필드명 중 있는 걸 골라 쓰고 메시지 wrapping 형태도 다양하게 시도하는 극도로 방어적인 코드라, 그 기억이 사실이었음을 코드 자체가 증명. 다만 그 지저분함은 "CCTV → 서버" 구간의 것이라, 서버 없이 카메라 브로드캐스트를 직접 받는 지금 구조에서 목업까지 일부러 더럽힐 필요는 없다고 판단 — 목업(Phase 0의 `event_broadcaster.cpp`)은 깨끗한 JSON 그대로 두고, `EventService`의 기존 방어적 파싱 로직을 재사용하는 쪽으로 결정(설계 원칙 4).

**상태.** 완료 (2026-09-17). `EventService`가 서버 설정 게이트 뒤에서만 생성되던 문제(Phase 3b 안정화 패치의 4.1과 같은 종류)를 같이 고쳐서, 서버 없이도 로컬 이벤트가 항상 동작하게 함. `MainScreen`에 이미 있던 5초 디바운스+burst 안전장치(원래부터 있던 화면 계층 코드, 무수정)가 그대로 살아있어 별도 배칭 로직 추가 불필요. 세부 내용은 `docs/dev_execution_plan.md`의 Phase 4 섹션 참고.
